...
public class Food implements Comparable<Food> {
	...
	if (result == 0) {
	    result = Integer.compare(this.origin, target.origin);
	}
	return result;
    }
	...
	return String.format("%-12s%s\n%-12s%s\n%-12s%b\n%-12s%d\n", "Name:", name, "Origin:", ORIGINS[origin],
		"Vegetarian:", isVegetarian, "Calories:", calories);
    }
	...
	String output = String.format("%s|%n|%b|%d", name, origin, isVegetarian, calories);
	ps.println(output);
